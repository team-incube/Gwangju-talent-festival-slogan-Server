...
public class GoogleSheetsAdapter {
    ...
    public GoogleSheetsAdapter(
            @Value("${google.sheets.private-sheet-id}") String privateSheetId,
            @Value("${google.sheets.private-sheet-page}") String privateSheetPage,
            @Value("${google.sheets.public-sheet-id}") String publicSheetId,
            @Value("${google.sheets.public-sheet-page}") String publicSheetPage,
            @Value("${google.account-credential}") String accountCredential
    ) {
        ...
        try {
            ...
            ByteArrayInputStream credentialsStream =
                    new ByteArrayInputStream(accountCredential.getBytes(StandardCharsets.UTF_8));
            ...
            throw new IllegalStateException("Failed to initialize Google Sheets client", e);
        }
    }

    public void appendSlogan(SubmitSloganRequest request, Long id, SheetType sheetType) {
        List<List<Object>> rows = getLists(request, id, sheetType);
        appendRows(rows, sheetType);
    }

    public void appendRows(List<List<Object>> rows, SheetType sheetType) {
